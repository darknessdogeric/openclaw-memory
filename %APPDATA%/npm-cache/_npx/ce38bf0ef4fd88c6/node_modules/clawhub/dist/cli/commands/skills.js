...
export async function cmdInstall(opts, slug, versionFlag, force = false) {
    ...
    try {
        ...
        const skillMeta = await apiRequest(registry, { method: 'GET', path: `${ApiRoutes.skills}/${encodeURIComponent(trimmed)}` }, ApiV1SkillResponseSchema);
...
export async function cmdUpdate(opts, slugArg, options, inputAllowed) {
    ...
    for (const entry of slugs) {
        ...
        try {
            ...
            const skillMeta = await apiRequest(registry, { method: 'GET', path: `${ApiRoutes.skills}/${encodeURIComponent(entry)}` }, ApiV1SkillResponseSchema);
            // Check moderation status before proceeding
            if (skillMeta.moderation?.isMalwareBlocked) {
