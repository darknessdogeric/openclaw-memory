...
describe('cmdBanUser', () => {
    ...
    it('includes reason when provided', async () => {
        ...
        await cmdBanUser(makeOpts(), 'hightower6eu', { yes: true, reason: 'malware distribution' }, false);
        expect(mockApiRequest).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            method: 'POST',
            path: '/api/v1/users/ban',
    ...
    it('resolves user via fuzzy search', async () => {
        ...
        expect(mockApiRequest).toHaveBeenNthCalledWith(1, expect.anything(), expect.objectContaining({
            method: 'GET',
            path: expect.stringContaining('/api/v1/users?'),
        }), expect.anything());
        expect(mockApiRequest).toHaveBeenNthCalledWith(2, expect.anything(), expect.objectContaining({
            method: 'POST',
            path: '/api/v1/users/ban',
    ...
    it('fails fuzzy search with multiple matches when not interactive', async () => {
        ...
        await expect(cmdBanUser(makeOpts(), 'moonshine', { yes: true, fuzzy: true }, false)).rejects.toThrow(/multiple users matched/i);
    });
});
describe('cmdSetRole', () => {
    it('requires --yes when input is disabled', async () => {
    ...
    it('rejects invalid roles', async () => {
        await expect(cmdSetRole(makeOpts(), 'demo', 'owner', { yes: true }, false)).rejects.toThrow(/role/i);
    });
    it('posts handle payload', async () => {
